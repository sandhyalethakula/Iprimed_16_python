...
Enter > ''')                                                            #Ask user input to perform specific task in list mentioned
        ...
        if len(student_details) == 0:
            ...
            for i in student_details:                                   #iterate each value in student_details, convert the details into list and print with comma separated
                details = list(i)
                print("        "," ".join(details[:len(details)-1]))
                print()
        choose_a_number = ''  
        break
 ...
 payment mode:
        ...
        if course in "1234" and payment != '' and name!= '':                         #if users enters correct details a new tuple is created and append to studet_details list
            user_id = len(student_details)+1
            student_details.append((str(user_id),name,courses[int(course)-1],str(fee),payment))         
...
Enter here > '''))
        ...
        if len(student_details) == 0 or userId > len(student_details):               #if details no found raises error
            ...
            for i in range(len(student_details)):
                num+=1
                data = list(student_details[i])
                data[0] = str(num)
                student_details[i]=tuple(data)
            print("\t",delete_details[:len(delete_details)-1],"is deleted from the list")
            choose_a_number =''
...
Enter here> '''))                                                                   #Asks user to enter userId to edit
    ...
    Enter here > ''')                                                               #Asks the user which has to edit
            if what_to_update == "1":                                               #If user selects both payment & course details it process
                print("~"*15)
                print("    Your are editing for", " ".join(details[:len(details)-1]))
                print()
                course = input('''
         Courses & Fees:
            ...
            Enter paymode number > ''')                                             #Ask payment mode
                if course == "1":                                                   #checks the course name and procede to fee
                    ...
                    if payment == "1":
                        ...
                        fee = 20000 - (20000*5/100)
                if course in "1234" and payment != '' and name!= '':                #if users enters correct details a new tuple is created and append to studet_details list
                    student_details[userId-1] = (str(userId),name,courses[int(course)-1],str(fee),payment)
                    print("         Your details are edited")
                    choose_a_number = ''
                else:
                    print("Wrong info please enter correct details")
            elif what_to_update == "2":                                             #Checks user selected number is match with payment mode 
                print("~"*15)
                print("    Your are editing for"," ".join(details[:len(details)-1]))
                print()
                if "2" in details[4]:
 ...
 payment mode:
        ...
        Enter paymode number > ''')                                                     #asks the user to select payment mode to update
                if payment == "1":
                    if details[2] == "Python":
                        details[3]= str(15000 +500)                                     #according to course payment & payment mode fee is updated
                        ctr=1                                                           #by default c is 0  after c =1 to indicates that the fee is updated
                    elif details[2] == "Java":
                        details[3] = str(8000 + 500)
                        ctr=1
                    elif details[2] == "Ruby":
                        details[3] = str(10000+500)
                        ctr=1
                    elif details[2] == "Rust":
                        details[3] = str(20000+500)
                        ctr=1
                else:
                    if details[2] == "Python":
                        details[3]= str(15000 -(15000*5/100))
                        ctr=1
                    elif details[2] == "Java":
                        details[3] = str(8000 -(8000*5/100))
                        ctr=1
                    elif details[2] == "Ruby":
                        details[3] = str(10000 -(10000*5/100))
                        ctr=1
                    elif details[2] == "Rust":
                        details[3] = str(20000 -(20000*5/100))
                        ctr=1 
                if ctr != 0:                                                        #the c value is updated that indicates payment mode updated and the data stored in list
         ...
         Courses & Fees:
            ...
            Enter a new course >''')
                if course == "1":                                                   #checks the course and update the payment according to it
                    details[2]= "Python"
                    if "." in details[3]:
                        details[3] = str(15000 - (15000*5/100))                     #after change course selected the course is upated
                    else:
                        details[3]= str(15000+500)
                    ctr=1                                                           #by default c is 0  after c =1 to indicates that the couse is updated
                elif course == "2":
                    details[2]= "Java"
                    if "." in details[3]:
                        details[3] = str(8000 - (8000*5/100))
                    ...
                    if "." in details[3]:
                        ...
                        details[3]= str(20000+500)
                    ctr=1
                if ctr != 0:                                                #the c value is updated that indicates course updated in list data 
                    student_details[userId-1] = tuple(details)
                    print("\t","   Details are updated!")
                    choose_a_number=''
    ...
    Enter here > ''')                                                       #Asks user to select Card or E-wallet as Filter
            print("~"*60)
            for i in student_details:
                if choose_payment_mode == i[4]:                             #checks data is matched with Card payment mode or not
                    print("\t"," ".join(list(i[:len(i)-1])))
                elif i[4] == choose_payment_mode:
                    print("\t"," ".join(list(i[:len(i)-1])))                #checks data is matched with Card payment mode or not
            print("         Filtered by {} payment method".format(pym[int(choose_payment_mode)-1]))
            choose_a_number =''
    elif choose_a_number == "6":                                            #Checks user selected number is matched to find a student
        ctr=0
        print("~"*30)
        print("        FIND selected")                                      #Indicates FIND selected
        enter_name = input('''
        Enter Name or Part Name to find student: > ''')                     #Asks user to enter name or part of name
        for i in student_details:
            if enter_name in i[1]:                                          #Iterate each details in student_details and checks where input words are in each details
                print("\t"," ".join(list(i[:len(i)-1])))                    #If user input is found in the list it prints the details of student
                ctr=1
        if ctr == 0:
            print("*"*20)
            print("        Not Found!")                                     #If user input doesn't match it prints not found
        else:
            choose_a_number= ''
